...
const defaultOrigins = isDev   ? 'https://localhost:4422'
                     : isStage ? 'https://localhost:4244'
                     :           'https://fl-legal.ca';
 
// Default TLS cert/key paths based on environment tier.
// Can always be overridden via TLS_CERT / TLS_KEY env vars.
const defaultCert = isDev  ? 'cert/local/localhost.crt'
                 : isStage ? 'cert/stage/staging.cert'
                 :           '';   // production: provide TLS_CERT explicitly
 
const defaultKey  = isDev  ? 'cert/local/localhost.decrypted.key'
                 : isStage ? 'cert/stage/staging.key'
                 :           '';   // production: provide TLS_KEY explicitly
