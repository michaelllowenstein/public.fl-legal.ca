...
//   - currentUser() returns AuthUser | null, typed to cover both roles
...
export type TokenPayload = LawyerTokenPayload | EditorTokenPayload;
 
// ── Public user model exposed to consumers ────────────────────────────────────
 
export interface AuthUser {
  role: 'lawyer' | 'editor';
  /** Firebase UID for lawyers; editor ID (or 'editor') for editors */
  uid: string;
  email?: string;
  displayName?: string;
...
export class AuthService {
  ...
  token(): string | null {
    return this._lawyerToken() ?? this._editorToken();
  }
  ...
  logout(role: 'lawyer' | 'editor' | 'all' = 'all'): void {
   ...
   * Lawyer takes precedence — a lawyer session masks any editor session.
   */
